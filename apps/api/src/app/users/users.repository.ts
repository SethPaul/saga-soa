...
export class UsersRepository {
  ...
  async findById(id: string): Promise<User | null> {
    const db = (this.mongoProvider as MongoDbService).getDatabase();
    return await db.collection(this.collectionName).findOne({ _id: new ObjectId(id) }) as User | null;
  }

  async findByEmail(email: string): Promise<User | null> {
    const db = (this.mongoProvider as MongoDbService).getDatabase();
    return await db.collection(this.collectionName).findOne({ email }) as User | null;
  }
  ...
  async update(id: string, updateData: Omit<UpdateUserDto, 'configType'>): Promise<User | null> {
    const db = (this.mongoProvider as MongoDbService).getDatabase();
